...
import com.gw.component_website.api.interfaces.IWebsiteManagerApi
...
class FamilyFragment : ABaseMVVMDBFragment<FamilyFragmentFamilyBinding, FamilyVM>() {
    ...
    companion object {
         ...
         * 毛玻璃效果的模糊度
         */
        private const val BLUR_NUMBER = 16F
    }
    ...
    @Inject
    lateinit var appConfigApi: IAppConfigApi

    @Inject
    lateinit var configApi: IConfigApi

    @Inject
    lateinit var websiteManagerApi: IWebsiteManagerApi
...
//        setNoticeView()
    ...
    override fun initLiveData(viewModel: FamilyVM, savedInstanceState: Bundle?) {
                    ...
                    when (action) {
                        ...
                        is HttpAction.Success -> {
                            ...
                            if (data != null) {
                                ...
                                GwellLogUtils.i(TAG, "onScanShareDevice: showCommDialog devId==${data.devId} remarkName==${data.remarkName}")
                            }
                        }
                        // 失败分情况弹窗
                        is HttpAction.Fail -> {
        ...
        viewModel.mHistoryDeviceList.observe(this) {
            if (it.isNotEmpty()) {
                var contentStr = (getString(RR.string.AA0662))
                it.forEachIndexed { index, deviceBean ->
                    contentStr += "\n ${index + 1}.${deviceBean.deviceName}"
                }
                contentStr += "\n${getString(RR.string.AA0663)}"
                showCommDialog {
                    title = getString(RR.string.AA0661)
                    content = TextContent(contentStr)
                    actions = listOf(
                        CommDialogAction(getString(RR.string.AA0664)),
                        CommDialogAction(
                            text = getString(RR.string.AA0660),
                            onClick = {
                                val downloadUrl = websiteManagerApi.getOldXiaotunDownloadUrl()
                                webViewApi.openWebView(downloadUrl, "")
                                GwellLogUtils.i(TAG, "openWebView: downloadUrl == $downloadUrl")
                            }
                        ),
                    )
                }
        ...
        override fun onReceiveOnlineMsg(msg: String, topic: String) {
            ...
            when (topic) {
                ...
                EventTopicType.NOTIFY_USER_PWD_CHANGE -> {
                    ...
                    GwellLogUtils.i(TAG, "onReceiveOnlineMsg else msg = $msg topic = $topic")
                }
            }
        }
